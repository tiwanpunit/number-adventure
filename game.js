(() => {
  ...
  function recommendedStage(){return IS_LOCAL_TEST?progress.lastStage:Math.min(progress.lastStage,progress.highestUnlocked)}
  function updateHomeButton(){const stage=recommendedStage(),hasProgress=progress.completed.length>0||progress.lastStage>0;$('#playBtn').textContent=hasProgress?`▶ Continue Stage ${stage+1}`:'▶ Start Adventure'}
  function show(id){screens.forEach(s=>s.classList.toggle('active',s.id===id));if(id!=='gameScreen')stopLoop();if(id==='homeScreen')updateHomeButton()}
  ...
  function startStage(i){if(!IS_LOCAL_TEST&&i>progress.highestUnlocked)return;currentStageIndex=i;progress.lastStage=i;saveProgress();updateHomeButton();show('gameScreen');resetGame()}
  ...
  function finishStage(){
    stopLoop();if(!progress.completed.includes(currentStageIndex))progress.completed.push(currentStageIndex);progress.starsByStage[currentStageIndex]=Math.max(progress.starsByStage[currentStageIndex]||0,stars);if(currentStageIndex<STAGES.length-1)progress.highestUnlocked=Math.max(progress.highestUnlocked,currentStageIndex+1);progress.lastStage=Math.min(STAGES.length-1,currentStageIndex+1);saveProgress();renderStageSelect();updateHomeButton();
  ...
  $('#playBtn').addEventListener('click',()=>startStage(recommendedStage()));$('#chooseStagesBtn').addEventListener('click',()=>{renderStageSelect();show('stageScreen')});$('#stageBackBtn').addEventListener('click',()=>show('homeScreen'));
  ...
  $('#homeZero').innerHTML=zeroHTML();$('#homeOne').innerHTML=numberHTML(1);renderStageSelect();updateHomeButton();
